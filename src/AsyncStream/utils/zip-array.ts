...
export function zipArray<B>(
	fb: AsyncStream<B>,
): <A>(fa: AsyncStream<A>) => AsyncStream<readonly [A, B]>;
...
export function zipArray<A, B>(
	fa: AsyncStream<A>,
	fb: AsyncStream<B>,
): AsyncStream<readonly [A, B]>;
export function zipArray<A, B>(
	fAOrB: AsyncStream<A> | AsyncStream<B>,
	fb?: AsyncStream<B>,
):
	| AsyncStream<readonly [A, B]>
	| (<A>(fa: AsyncStream<A>) => AsyncStream<readonly [A, B]>) {
...
export function unzipArray<A, B>(
	mma: AsyncStream<readonly [A, B]>,
): [AsyncStream<A>, AsyncStream<B>] {
	return [
		pipe(
			mma,
			map((it) => it[0]),
