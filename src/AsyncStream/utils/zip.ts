...
export function zip<B>(
	fb: AsyncStream<B>,
): <A>(fa: AsyncStream<A>) => AsyncStream<AsyncStream<A | B>>;
...
export function zip<A, B>(
	...
	| AsyncStream<AsyncStream<A | B>>
	| (<A>(ma: AsyncStream<A>) => AsyncStream<AsyncStream<A | B>>) {
	if (typeof fb !== "undefined") {
		return pipe(zipArray(fAOrB, fb), map(fromIterable));
	}
...
export function unzip<A, B>(
	mma: AsyncStream<AsyncStream<A> | AsyncStream<B>>,
): AsyncStream<A | B> {
