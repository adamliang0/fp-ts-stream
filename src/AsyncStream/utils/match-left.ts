 ...
 * @__PURE__
 */
export function matchLeftW<B, A, C>(
	onEmpty: Lazy<B> | Task<B>,
	onNonEmpty: (head: A, tail: AsyncStream<A>) => C | Promise<C>,
) {
	/**
	...
	return function _matchLeftW(fa: AsyncStream<A>): Task<B | C> {
		return async function __matchLeftW() {
			...
			if (done) {
				...
				return await onNonEmpty(head, async function* _tail() {
					yield* gen;
				});
			}
		};
 ...
 * @__PURE__
 */
export function matchLeft<B, A>(
	onEmpty: Lazy<B> | Task<B>,
	onNonEmpty: (head: A, tail: AsyncStream<A>) => B | Promise<B>,
) {
