...
export function traverse<F>(
	F: Applicative<F>,
): <A, B>(f: (a: A) => HKT<F, B>) => (ta: Kind<URI, A>) => HKT<F, Kind<URI, B>>;
export function traverse<F>(F: Applicative<F>) {
	...
	return function _traverse<A, B>(f: (a: A) => HKT<F, B>) {
		...
		return function __traverse(ta: Kind<URI, A>): HKT<F, Kind<URI, B>> {
			...
			for (const a of ta()) {
				const mapped = F.map(fb, (sb: Stream<B>) => (b: B) => append(b)(sb));
...
export function sequence<F extends URIS4>(
	...
	ta: Kind<URI, Kind4<F, S, R, E, A>>,
) => Kind4<F, S, R, E, Kind<URI, A>>;
export function sequence<F extends URIS3>(
	F: Applicative3<F>,
): <R, E, A>(ta: Kind<URI, Kind3<F, R, E, A>>) => Kind3<F, R, E, Kind<URI, A>>;
export function sequence<F extends URIS3, E>(
	F: Applicative3C<F, E>,
): <R, A>(ta: Kind<URI, Kind3<F, R, E, A>>) => Kind3<F, R, E, Kind<URI, A>>;
export function sequence<F extends URIS2>(
	F: Applicative2<F>,
): <E, A>(ta: Kind<URI, Kind2<F, E, A>>) => Kind2<F, E, Kind<URI, A>>;
export function sequence<F extends URIS2, E>(
	F: Applicative2C<F, E>,
): <A>(ta: Kind<URI, Kind2<F, E, A>>) => Kind2<F, E, Kind<URI, A>>;
export function sequence<F extends URIS>(
	F: Applicative1<F>,
): <A>(ta: Kind<URI, Kind<F, A>>) => Kind<F, Kind<URI, A>>;
export function sequence<F>(
	F: Applicative<F>,
): <A>(ta: Kind<URI, HKT<F, A>>) => HKT<F, Kind<URI, A>>;
