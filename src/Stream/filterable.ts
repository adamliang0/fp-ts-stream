...
): <B extends A>(fa: Stream<B>) => Stream<B>;
...
export function partition<A>(
	predicate: Predicate<A>,
): <B extends A>(fb: Stream<B>) => Separated<Stream<B>, Stream<B>>;
...
export function partition<A>(
	predicate: Predicate<A>,
): (fb: Stream<A>) => Separated<Stream<A>, Stream<A>>;
export function partition<A>(
	predicate: Predicate<A>,
): (fb: Stream<A>) => Separated<Stream<A>, Stream<A>> {
...
export function partitionWithIndex<A>(
	predicateWithIndex: PredicateWithIndex<number, A>,
): (fa: Stream<A>) => Separated<Stream<A>, Stream<A>>;
export function partitionWithIndex<A>(
	predicateWithIndex: PredicateWithIndex<number, A>,
): (fa: Stream<A>) => Separated<Stream<A>, Stream<A>> {
	/**
	...
	return function _partitionWithIndex(fa) {
		return pipe(
			fa,
			mapWithIndex((i, it) =>
				predicateWithIndex(i, it) ? right(it) : left(it),
			),
			separate,
		);
	};
...
export const Filterable: Filterable1<URI> = {
	...
	filter<A>(fa: Stream<A>, p: Predicate<A>) {
		return filter(p)(fa);
	},
	filterMap(fa, f) {
		return filterMap(f)(fa);
	},
	partition<A>(fa: Stream<A>, p: Predicate<A>) {
