...
import type { HKT, Kind, Kind2, Kind3, URIS, URIS2, URIS3 } from "fp-ts/HKT";
...
) => (ta: Kind<URI, A>) => HKT<F, Kind<URI, B>>;
export function traverseWithIndex<F>(F: Applicative<F>) {
	...
	return function _traverseWithIndex<A, B>(f: (i: number, a: A) => HKT<F, B>) {
		 ...
		 * @__PURE__
		 */
		return function __traverseWithIndex(
			ta: Kind<URI, A>,
		): HKT<F, Kind<URI, B>> {
			let fb = F.of(zero<B>());
			let index = 0;
...
export const TraversableWithIndex: TraversableWithIndex1<URI, number> = {
	...
	traverseWithIndex<F>(F: Applicative<F>) {
		return function _traverseWithIndex<A, B>(
			ta: Stream<A>,
			f: (i: number, a: A) => HKT<F, B>,
		) {
